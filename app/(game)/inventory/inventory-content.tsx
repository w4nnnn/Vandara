...
import { useItem, sellItem, buyItem } from '@/app/actions/inventory'
...
const SHOP_ITEMS = Object.values(ITEMS).filter((i) => i.category !== 'material')
...
export default function InventoryContent({
  ...
  function handleBuy(itemId: string) {
    startTransition(async () => {
      const res = await buyItem(itemId, 1)
      if (res.error) setMessage({ text: res.error, type: 'error' })
      else
        setMessage({
          text: `Bought for $${res.totalCost}`,
          type: 'success',
        })
      router.refresh()
    })
      ...
      <Tabs defaultValue="inventory">
        <TabsList className="w-full">
          <TabsTrigger value="inventory" className="flex-1">
            <BackpackIcon className="mr-1.5 size-4" />
            Inventory
          </TabsTrigger>
        ...
        <TabsContent value="shop" className="mt-4">
          <Card>
            ...
            <CardContent className="space-y-3">
              {SHOP_ITEMS.map((def) => {
                const CatIcon = CATEGORY_ICON[def.category]
                const canAfford = Number(player.money) >= def.value
                return (
                  <div
                    key={def.id}
                    className="flex items-center justify-between rounded-lg border p-3"
